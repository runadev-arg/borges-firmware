...
bool ensureTrustedClock(std::string* detail) {
  ...
  if (BORGES_QUEUE.begin() &&
      BORGES_QUEUE.checkpoint().wallAnchorUnixSeconds >= static_cast<uint64_t>(netboot::MIN_TRUSTED_TIME)) {
    const timeval approximate{
        .tv_sec = static_cast<time_t>(BORGES_QUEUE.checkpoint().wallAnchorUnixSeconds),
        .tv_usec = 0,
    };
  ...
  const std::string host = netboot::hostFromBaseUrl(BORGES_CREDENTIALS.getBaseUrl());
  if (host.empty()) {
  ...
  for (uint8_t index = 0; index < candidates.count; ++index) {
    ...
    LOG_INF("BORGES", "Clock bootstrapped from HTTP Date header");
    return true;
  }
  ...
  for (uint8_t index = 0; index < candidates.count; ++index) {
    ...
    LOG_INF("BORGES", "Clock bootstrapped from HTTPS Date header");
    return true;
  }
  if (detail != nullptr) *detail = candidates.detail + " " + dateDetail + " " + httpsDetail;
  return false;
