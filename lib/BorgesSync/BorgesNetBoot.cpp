...
constexpr char BAKED_HOST[] = "borges.runadev.com";
...
Candidates resolveServer(const char* host) {
  ...
  IPAddress cached;
  if (sameHostCaseInsensitive(BORGES_CREDENTIALS.getServerIpHost(), host) &&
      cached.fromString(BORGES_CREDENTIALS.getServerIp().c_str())) {
...
void rememberServerIp(const char* host, IPAddress ip) {
  if (host == nullptr || *host == '\0') return;
  BORGES_CREDENTIALS.setServerIp(host, std::string(ip.toString().c_str()));
}
...
uint64_t httpDateEpoch(IPAddress ip, const char* host, std::string& detail) {
  ...
  request += host;
  request += "\r\nUser-Agent: Borges-Borges/" BORGES_VERSION "\r\nAccept: */*\r\nConnection: close\r\n\r\n";
