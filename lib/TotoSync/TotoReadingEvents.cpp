...
ResolvedTime ReadingEvents::now() const {
  ...
  return resolveWallTime(TOTO_QUEUE.checkpoint(), bootId, millis());
}

bool ReadingEvents::enqueueStarted() {
  const auto identity = TOTO_QUEUE.reserveIdentity(deterministicUuid(sessionId + ":session.started"));
  ...
  const bool queued = !event.empty() && TOTO_QUEUE.enqueue(*identity, event);
  if (queued) TOTO_SYNC_SCHEDULER.notifyLifecycleCommit();
  return queued;
}
...
bool ReadingEvents::persistProgress(float percentage, const std::string& xpointer, bool explicitJump) {
  ...
  if (TOTO_QUEUE.replaceUnattempted(progressIdentity, event)) {
    TOTO_SYNC_SCHEDULER.notifyLifecycleCommit();
    return true;
  }

  const auto replacement = TOTO_QUEUE.reserveIdentity();
  if (!replacement) return false;
  progressIdentity = *replacement;
  const bool queued = TOTO_QUEUE.enqueue(
...
bool ReadingEvents::persistPageStats() {
  ...
  if (pageStatsIdentity.id.empty()) {
    const auto identity =
        TOTO_QUEUE.reserveIdentity(deterministicUuid(sessionId + ":page_stats:" + std::to_string(statsBucket)));
    if (!identity) return false;
  ...
  if (event.empty()) return false;
  if (TOTO_QUEUE.replaceUnattempted(pageStatsIdentity, event)) {
    TOTO_SYNC_SCHEDULER.notifyLifecycleCommit();
    return true;
  }
...
void ReadingEvents::persistEnded(uint32_t totalDurationSeconds) {
  ...
  while (remaining >= MIN_SESSION_SECONDS) {
    ...
    const auto identity =
        TOTO_QUEUE.reserveIdentity(deterministicUuid(sessionId + ":session.ended:" + std::to_string(chunkIndex)));
    if (!identity) return;
    ...
    const std::string event =
        serializeCommon(*identity, "session.ended", "reading_session", sessionId + ":" + std::to_string(chunkIndex),
                        payload, wall.unixSeconds, wall.precision);
    if (event.empty() || !TOTO_QUEUE.enqueue(*identity, event)) return;
    TOTO_SYNC_SCHEDULER.notifyLifecycleCommit();
    remaining -= duration;
    offset += duration;
    ++chunkIndex;
