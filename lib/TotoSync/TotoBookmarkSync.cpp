...
ResolvedTime eventTime() {
  ...
  return resolveWallTime(TOTO_QUEUE.checkpoint(), TOTO_QUEUE.getBootId(), millis());
}
...
std::string serializeEvent(const EventIdentity& identity, const BookContext& book, const BookmarkEntry& bookmark,
  ...
  event["aggregate_id"] = bookmark.totoSyncId;
  ...
  payload["sync_id"] = bookmark.totoSyncId;
  payload["base_revision"] = std::to_string(bookmark.totoRevision);
  if (eventType != "bookmark.deleted") {
    ...
    if (!bookmark.totoNote.empty()) payload["note"] = boundedUtf8(bookmark.totoNote, MAX_NOTE_BYTES);
...
bool enqueueUpsert(const BookContext& book, BookmarkEntry& bookmark, std::string passage) {
  const uint32_t progress = basisPoints(bookmark.percentage);
  if (bookmark.totoSyncId.empty()) {
    bookmark.totoSyncId = legacyBookmarkSyncId(book.hash, bookmark.xpath, progress);
  }
  ...
  const std::string fingerprint =
      annotationFingerprint(bookmark.totoSyncId, passage, bookmark.totoNote, bookmark.xpath, progress);
  if (bookmark.totoFingerprint == fingerprint && !bookmark.totoConflict && bookmark.totoPendingEventId.empty()) {
    return true;
  }

  if (isUuid(bookmark.totoPendingEventId) && bookmark.totoPendingSequence > 0) {
    const EventIdentity pending{.id = bookmark.totoPendingEventId, .sequence = bookmark.totoPendingSequence};
    const std::string event = serializeEvent(pending, book, bookmark, "bookmark.upserted", passage);
    if (!event.empty() && TOTO_QUEUE.replaceUnattempted(pending, event)) {
      bookmark.totoFingerprint = fingerprint;
      bookmark.totoConflict = false;
      TOTO_SYNC_SCHEDULER.notifyLifecycleCommit();
      return true;
    }
  }

  const auto identity = TOTO_QUEUE.reserveIdentity();
  if (!identity) return false;
  const std::string event = serializeEvent(*identity, book, bookmark, "bookmark.upserted", passage);
  if (event.empty() || !TOTO_QUEUE.enqueue(*identity, event)) return false;
  bookmark.totoPendingEventId = identity->id;
  bookmark.totoPendingSequence = identity->sequence;
  bookmark.totoFingerprint = fingerprint;
  bookmark.totoConflict = false;
  TOTO_SYNC_SCHEDULER.notifyLifecycleCommit();
  return true;
...
bool applyAnnotation(const AnnotationInboxItem& item, std::vector<BookmarkEntry>& bookmarks, bool& changed) {
  auto existing = std::find_if(bookmarks.begin(), bookmarks.end(),
                               [&item](const BookmarkEntry& bookmark) { return bookmark.totoSyncId == item.syncId; });
  if (item.conflict) {
    if (existing != bookmarks.end()) {
      existing->totoConflict = true;
      existing->totoRevision = item.revision;
      existing->totoPendingEventId.clear();
      existing->totoPendingSequence = 0;
      changed = true;
    }
    return true;
  }
  ...
  bookmark.totoSyncId = item.syncId;
  bookmark.totoRevision = item.revision;
  bookmark.totoPendingEventId.clear();
  bookmark.totoPendingSequence = 0;
  bookmark.totoConflict = false;
  ...
  bookmark.totoNote = boundedUtf8(item.note, MAX_NOTE_BYTES);
...
bool BookmarkSync::reconcile(const std::string& epubPath, const std::string& title, const std::string& author,
  ...
  if (!TOTO_QUEUE.begin()) return false;
  ...
  for (size_t count = 0; count < MAX_APPLY_PER_OPEN; ++count) {
    const auto item = TOTO_QUEUE.nextAnnotation(book->hash, afterSequence);
    if (!item) break;
    if (!applyAnnotation(*item, bookmarks, changed)) {
      LOG_ERR("TOTO", "Unsupported annotation directive '%s'", item->directive.c_str());
      break;
    }
  ...
  if (TOTO_CREDENTIALS.paired()) {
    for (BookmarkEntry& bookmark : bookmarks) {
      if (bookmark.totoSyncId.empty()) {
        bookmark.totoSyncId = legacyBookmarkSyncId(book->hash, bookmark.xpath, basisPoints(bookmark.percentage));
        changed = true;
      }
      if (bookmark.totoFingerprint.empty() && enqueueUpsert(*book, bookmark, bookmark.summary)) {
        changed = true;
      }
    }
  }
  ...
  if (changed && (persist == nullptr || !persist(epubPath, bookmarks))) {
    LOG_ERR("TOTO", "Bookmark state was not persisted; retaining annotation inbox");
    return false;
  }
  for (uint64_t sequence : processedSequences) {
    if (!TOTO_QUEUE.resolveInbox(sequence)) {
      LOG_ERR("TOTO", "Could not retire annotation inbox item %llu", static_cast<unsigned long long>(sequence));
      return changed;
    }
  }
  return changed;
...
bool BookmarkSync::enqueueLocalUpsert(const std::string& epubPath, const std::string& title, const std::string& author,
  ...
  if (!TOTO_CREDENTIALS.paired()) return true;
  if (!TOTO_QUEUE.begin()) return false;
  const auto book = contextFor(epubPath, title, author);
...
bool BookmarkSync::enqueueLocalDelete(const std::string& epubPath, const std::string& title, const std::string& author,
  ...
  if (!TOTO_CREDENTIALS.paired() || bookmark.totoSyncId.empty()) return true;
  if (!TOTO_QUEUE.begin()) return false;
  const auto book = contextFor(epubPath, title, author);
  if (!book) return false;
