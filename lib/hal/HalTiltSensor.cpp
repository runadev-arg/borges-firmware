...
void HalTiltSensor::update(const uint8_t mode, const uint8_t orientation, const bool inReader) {
  ...
  if ((mode != BorgesTiltPageTurn::TILT_OFF) && !_isAwake) {
    _isAwake = wake();
    return;
  } else if ((mode == BorgesTiltPageTurn::TILT_OFF) && _isAwake) {
    _isAwake = !deepSleep();
    return;
  }

  // If disabled, skip the rest of the polling logic and avoid unnecessary I2C traffic in non-reader activities
  if ((mode == BorgesTiltPageTurn::TILT_OFF) || !inReader) {
    return;
  }
  ...
  float tiltAxis;
  switch (orientation) {
    case BorgesOrientation::PORTRAIT:
      tiltAxis = mode == BorgesTiltPageTurn::TILT_INVERTED ? -gx : gx;
      break;
    case BorgesOrientation::INVERTED:
      tiltAxis = mode == BorgesTiltPageTurn::TILT_INVERTED ? gx : -gx;
      break;
    case BorgesOrientation::LANDSCAPE_CW:
      tiltAxis = mode == BorgesTiltPageTurn::TILT_INVERTED ? gy : -gy;
      break;
    case BorgesOrientation::LANDSCAPE_CCW:
      tiltAxis = mode == BorgesTiltPageTurn::TILT_INVERTED ? -gy : gy;
      break;
    default:
      tiltAxis = gx;
