...
OUTPUT_EPUB = ROOT / "Borges_User_Guide.epub"
...
def split_chapters(html: str):
    ...
    if not positions:
        return [('intro', 'Borges User Guide', html)]
...
def build_epub():
    ...
    book.set_title('Borges User Guide')
    book.set_language('en')
    book.add_author('Borges Reader Project')
    ...
    # Full-size cover image (480×800) — used by Borges for home-screen thumbnail.
    ...
    # EPUB 2 cover declaration — Borges's Tier 1 OPF lookup
    book.add_metadata('OPF', 'meta', '', {'name': 'cover', 'content': 'cover-image'})

    # Cover page XHTML — first readable page in the spine
    cover_page = epub.EpubHtml(title='Borges User Guide', file_name='cover.xhtml', lang='en')
    cover_page.content = make_xhtml(
        'Borges User Guide',
        '<div class="cover-page">'
        '<img src="images/cover.png" alt="Borges User Guide cover"/>'
        '</div>',
    )
    ...
    # nav is excluded from the spine — Borges locates it via properties="nav" in
