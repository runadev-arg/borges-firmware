...
Creates BMP files at 480x800 (Borges display in portrait orientation).
...
- 2-bit: 4-level grayscale (non-standard Borges extension, won't open on PC)
...
def generate_2bit(path):
    """2-bit BMP: 4-level grayscale test pattern (non-standard, Borges extension)."""
    bpp = 2
    palette = GRAY_LEVELS
    row_bytes = (WIDTH * bpp + 31) // 32 * 4
    palette_size = len(palette) * 4
...
def main():
    ...
    print()
    print("Note: 2-bit BMP is a non-standard Borges extension. Standard image viewers")
