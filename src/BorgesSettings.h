...
class BorgesSettings : public PersistableStore<BorgesSettings> {
 private:
  // Private constructor for singleton
  BorgesSettings() = default;

  friend class PersistableStore<BorgesSettings>;
 ...
 public:
  ...
  uint16_t getPowerButtonDuration() const {
    return (shortPwrBtn == BorgesSettings::SHORT_PWRBTN::SLEEP) ? 10 : 400;
  }
  ...
  static const char* getFilePath() { return "/.borges/settings.json"; }
  void toJson(JsonDocument& doc) const;
  bool fromJson(JsonVariantConst doc);

  static void validateFrontButtonMapping(BorgesSettings& settings);
...
#define SETTINGS BorgesSettings::getInstance()
