...
BorgesWebServer* wsInstance = nullptr;
...
BorgesWebServer::BorgesWebServer() {}

BorgesWebServer::~BorgesWebServer() { stop(); }

void BorgesWebServer::begin() {
  if (running) {
  ...
  wsInstance = const_cast<BorgesWebServer*>(this);
  wsServer->begin();
...
void BorgesWebServer::abortWsUpload(const char* tag) {
...
void BorgesWebServer::handleClient() {
    ...
    if (packetSize > 0) {
      ...
      if (len > 0) {
        ...
        if (strcmp(buffer, "hello") == 0) {
          ...
          String message = "borges (on " + hostname + ");" + String(wsPort);
...
BorgesWebServer::WsUploadStatus BorgesWebServer::getWsUploadStatus() const {
...
void BorgesWebServer::handleRoot() const {
  sendHtmlContent(server.get(), HomePageHtml, sizeof(HomePageHtml));
  LOG_DBG("WEB", "Served root page");
}

void BorgesWebServer::handleJszip() const {
  server->sendHeader("Content-Encoding", "gzip");
  server->send_P(200, "application/javascript", jszip_minJs, jszip_minJsCompressedSize);
  LOG_DBG("WEB", "Served jszip.min.js");
}

void BorgesWebServer::handleNotFound() const {
...
void BorgesWebServer::handleStatus() const {
  // Get correct IP based on AP vs STA mode
  const String ipAddr = apMode ? WiFi.softAPIP().toString() : WiFi.localIP().toString();

  JsonDocument doc;
  doc["version"] = BORGES_VERSION;
  doc["ip"] = ipAddr;
...
void BorgesWebServer::scanFiles(const char* path, const std::function<void(FileInfo)>& callback) const {
...
bool BorgesWebServer::isEpubFile(const String& filename) const { return FsHelpers::hasEpubExtension(filename); }

void BorgesWebServer::handleFileList() const {
  sendHtmlContent(server.get(), FilesPageHtml, sizeof(FilesPageHtml));
}

void BorgesWebServer::handleFileListData() const {
...
void BorgesWebServer::handleDownload() const {
  if (!server->hasArg("path")) {
...
static bool flushUploadBuffer(BorgesWebServer::UploadState& state) {
...
void BorgesWebServer::handleUpload(UploadState& state) const {
...
void BorgesWebServer::handleUploadPost(UploadState& state) const {
  if (state.success) {
...
void BorgesWebServer::handleCreateFolder() const {
...
void BorgesWebServer::handleRename() const {
...
void BorgesWebServer::handleMove() const {
...
void BorgesWebServer::handleDelete() const {
...
void BorgesWebServer::handleSettingsPage() const {
  sendHtmlContent(server.get(), SettingsPageHtml, sizeof(SettingsPageHtml));
  LOG_DBG("WEB", "Served settings page");
}

void BorgesWebServer::handleGetSettings() const {
...
void BorgesWebServer::handlePostSettings() {
...
void BorgesWebServer::handleGetOpdsServers() const {
...
void BorgesWebServer::handlePostOpdsServer() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void BorgesWebServer::handleDeleteOpdsServer() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void BorgesWebServer::handleGetWifiNetworks() const {
...
void BorgesWebServer::handlePostWifiNetwork() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
// Uses POST (not HTTP DELETE) because ESP32 WebServer doesn't support DELETE with body.
void BorgesWebServer::handleDeleteWifiNetwork() {
  if (!server->hasArg("plain")) {
    server->send(400, "text/plain", "Missing JSON body");
    return;
  }
...
void BorgesWebServer::wsEventCallback(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  if (wsInstance) {
...
void BorgesWebServer::onWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  switch (type) {
...
void BorgesWebServer::handleFontsPage() const {
  sendHtmlContent(server.get(), FontsPageHtml, sizeof(FontsPageHtml));
  LOG_DBG("WEB", "Served fonts page");
}

void BorgesWebServer::handleFontList() const {
...
void BorgesWebServer::handleFontUploadData() {
  ...
  switch (upload.status) {
    case UPLOAD_FILE_START: {
      ...
      // this an attacker could supply "../../.borges/settings.json" as
...
void BorgesWebServer::handleFontUpload() {
  if (fontUpload.valid) {
...
void BorgesWebServer::handleFontDelete() {
