...
void BorgesBookSyncActivity::onEnter() {
  ...
  if (!BORGES_CREDENTIALS.paired()) {
    ...
    if (!wifi) {
      message = std::string(tr(STR_BORGES_FAILED)) + ": " +
                borges::SyncClient::resultName(borges::SyncClient::Result::LOW_MEMORY);
      requestUpdate();
      return;
    }
...
void BorgesBookSyncActivity::performStep() {
  if (mode != Mode::Sync) {
    ...
    if (result.result != borges::SyncClient::Result::OK) {
      ...
      if (result.httpCode > 0) message += " (HTTP " + std::to_string(result.httpCode) + ")";
...
void BorgesBookSyncActivity::render(RenderLock&&) {
  ...
  const char* heading =
      mode == Mode::Sync ? tr(STR_BORGES_SYNC_NOW)
