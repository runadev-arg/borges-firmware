...
void TotoBookSyncActivity::onEnter() {
  Activity::onEnter();
  if (!TOTO_CREDENTIALS.paired()) {
    message = tr(STR_TOTO_BOOK_PAIR_FIRST);
    ...
    if (!wifi) {
      message =
          std::string(tr(STR_TOTO_FAILED)) + ": " + toto::SyncClient::resultName(toto::SyncClient::Result::LOW_MEMORY);
      requestUpdate();
      return;
    }
    startActivityForResult(std::move(wifi), [this](const ActivityResult& result) {
      working = !result.isCancelled && WiFi.status() == WL_CONNECTED;
      if (!working) message = tr(STR_TOTO_WIFI_REQUIRED);
      requestUpdate();
    });
  }
  requestUpdate();
...
void TotoBookSyncActivity::performStep() {
  if (mode != Mode::Sync) {
    const auto result = toto::SyncClient::fetchProgress(bookHash, mode == Mode::LatestOther);
    working = false;
    if (result.result != toto::SyncClient::Result::OK) {
      message = std::string(tr(STR_TOTO_FAILED)) + ": " + toto::SyncClient::resultName(result.result);
    } else if (!result.found) {
      message = (mode == Mode::LatestOther ? tr(STR_TOTO_BOOK_NO_OTHER) : tr(STR_TOTO_BOOK_NO_PROGRESS));
    } else {
    ...
    const auto result = toto::SyncClient::syncOnce();
    ...
    if (result.result != toto::SyncClient::Result::OK) {
      ...
      working = false;
      message = tr(STR_TOTO_BOOK_REJECTED);
    } else {
      working = result.hasMore || result.queueDepth > 0;
      if (working && rounds >= 64) {
        working = false;
        message = tr(STR_TOTO_BOOK_MORE_PENDING);
      } else {
        char summary[128];
        std::snprintf(summary, sizeof(summary), tr(STR_TOTO_SYNC_RESULT), static_cast<unsigned>(sent),
...
void TotoBookSyncActivity::loop() {
...
void TotoBookSyncActivity::render(RenderLock&&) {
  renderer.clearScreen();
  const auto& metrics = UITheme::getInstance().getMetrics();
  const int width = renderer.getScreenWidth();
  const char* heading = mode == Mode::Sync
                            ? tr(STR_TOTO_SYNC_NOW)
                            : (mode == Mode::Latest ? tr(STR_TOTO_BOOK_FETCH_LATEST) : tr(STR_TOTO_BOOK_FETCH_OTHER));
  ...
  const std::string text = working ? std::string(tr(STR_TOTO_WORKING)) + "\n" + message : message;
  for (const auto& line :
