...
bool isRecentIndex(const uint16_t* recentImages, uint8_t recentPos, uint8_t recentFill, uint16_t idx,
  ...
  for (uint8_t i = 0; i < effectiveCount; i++) {
    const uint8_t slot =
        (recentPos + BorgesState::SLEEP_RECENT_COUNT - 1 - i) % BorgesState::SLEEP_RECENT_COUNT;
...
void pushRecentIndex(uint16_t* recentImages, uint8_t& recentPos, uint8_t& recentFill, uint16_t idx) {
  ...
  recentPos = (recentPos + 1) % BorgesState::SLEEP_RECENT_COUNT;
  if (recentFill < BorgesState::SLEEP_RECENT_COUNT) recentFill++;
}

}  // namespace

bool BorgesState::isRecentSleep(uint16_t idx, uint8_t checkCount) const {
  return isRecentIndex(recentSleepImages, recentSleepPos, recentSleepFill, idx, checkCount);
}

bool BorgesState::isRecentOverlaySleep(uint16_t idx, uint8_t checkCount) const {
  return isRecentIndex(recentOverlaySleepImages, recentOverlaySleepPos, recentOverlaySleepFill, idx, checkCount);
}

void BorgesState::pushRecentSleep(uint16_t idx) {
  pushRecentIndex(recentSleepImages, recentSleepPos, recentSleepFill, idx);
}

void BorgesState::pushRecentOverlaySleep(uint16_t idx) {
  pushRecentIndex(recentOverlaySleepImages, recentOverlaySleepPos, recentOverlaySleepFill, idx);
}

void BorgesState::toJson(JsonDocument& doc) const {
...
bool BorgesState::fromJson(JsonVariantConst doc) {
  ...
  if (recentOverlaySleepPos >= SLEEP_RECENT_COUNT) {
    recentOverlaySleepPos = actualOverlayCount > 0 ? recentOverlaySleepPos % BorgesState::SLEEP_RECENT_COUNT : 0;
  }
