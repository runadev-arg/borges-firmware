...
// Version of the IP-pin contract (see SecureHttpClient::setServerAddress).
// Consumers feature-test on this to stay buildable against an older SDK.
#define FREEINK_SECURENET_HAS_SERVER_ADDRESS 1
...
class SecureHttpClient {
 public:
  ...
  // Dial this address instead of resolving the host of the begin() URL. The
  // Host: header, the SNI extension and the certificate name check all keep
  // using the URL's host, so pinning an address never weakens verification --
  // a wrong address simply fails the handshake. Meant for networks whose
  // DNS-over-UDP is dead but whose TCP egress works.
  //
  // This is transport configuration (like setCACert): it is NOT cleared by
  // begin(). It binds to the host in effect when it is called (or to the first
  // host used, if there was no begin() yet); should a redirect move to another
  // host the pin goes dormant, so an address is never dialed with a foreign
  // Host/SNI. Passing 0.0.0.0 is equivalent to clearServerAddress().
  //
  // Call order is begin() first, setServerAddress() after.
  void setServerAddress(IPAddress ip) {
    _serverIp = ip;
    _hasServerIp = static_cast<uint32_t>(ip) != 0;
    _serverIpHost = _host;  // "" when begin() has not run yet: bound on first use
  }
  ...
  bool hasServerAddress() const { return _hasServerIp; }
  IPAddress serverAddress() const { return _serverIp; }
  ...
  int sendRequestOnce(const char* method, const uint8_t* payload, size_t payloadLen, const DataCallback& onData,
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
      ...
      if (!ensureConnected()) {
        _errorDetail = connectErrorDetail();
        return -1;
      }
      ...
      if (!readLine(*_conn, line, headerDeadline, shouldAbort)) {
        ...
        _errorDetail = _aborted ? "abort" : "status_line";
        return -1;
      }
  ...
  // Which stage a negative sendRequest() return came from ("dns", "tcp",
  // "tls:-188", "ca_load", "write", "status_line", ...). Empty when the last
  // request reached a status line. Meant for surfacing transport failures in
  // UI on devices where Serial is not accessible.
  const std::string& lastErrorDetail() const { return _errorDetail; }
 ...
 private:
  ...
  static bool sameHost(const std::string& a, const std::string& b) {
  ...
  // The pin only applies to the host it was bound to. A redirect to another
  // host leaves it dormant (not erased: if the chain comes back to the
  // original host, it applies again).
  bool pinActive() const { return _hasServerIp && (_serverIpHost.empty() || sameHost(_serverIpHost, _host)); }
  ...
  // it already, which the send path handles with its one-shot retry. The pin
  // state is part of the match: a socket opened to a resolved address must not
  // be reused once the caller pins a different one (and vice versa).
  bool connectionMatches() const {
    return _conn && _conn->connected() && _connHttps == (_scheme == "https") && _connHost == _host &&
           _connPort == _port && _connPinned == pinActive() && (!_connPinned || _connIp == _serverIp);
  }

  // Reuse the kept-alive connection when it matches, else (re)connect.
  //
  // The endpoint is ALWAYS resolved to an address here, pin or no pin, and
  // dialed with connect(IPAddress, port). NetworkClient::connect(const char*,
  // ...) is off-limits SDK-wide: it resolves through Network.hostByName, which
  // calls raw-lwIP dns_clear_cache() without the TCPIP core lock and panics the
  // device when an SNTP query is in flight (see SecureClient::resolveHostIPv4).
  bool ensureConnected() {
    if (connectionMatches()) return true;
    closeConnection();
    // _plainStage describes THIS connect attempt only; a stale "dns" from an
    // earlier attempt would otherwise mask the TLS stage in connectErrorDetail.
    _plainStage.clear();
    const bool pinned = pinActive();
    if (pinned && _serverIpHost.empty()) _serverIpHost = _host;  // bind the pin to the first host used

    IPAddress target;
    if (pinned) {
      target = _serverIp;
    } else if (!SecureClient::resolveHostIPv4(_host.c_str(), target)) {
      _plainStage = "dns";
      return false;
    }
    ...
    if (_scheme == "https") {
      ...
      _secure.setResolvedAddress(target);  // SNI and cert verification still use _host
      ...
      if (!_plain.connect(target, _port)) {
        _plainStage = "tcp";
        return false;
      }
      _conn = &_plain;
  ...
  // Builds the failure tag for a connect that never got a socket up. Name
  // resolution runs before either transport, so its failure ("dns") is reported
  // ahead of anything the SecureClient could know; past that, https gets the
  // TLS stage and the wolfSSL code from SecureClient, and plain http can only
  // have failed the TCP connect.
  std::string connectErrorDetail() {
    if (_scheme != "https") return _plainStage.empty() ? "tcp" : _plainStage;
    if (!_plainStage.empty()) return _plainStage;  // resolution died before TLS started
    std::string detail = _secure.lastErrorStage();
    if (detail.empty()) detail = "connect";
    if (_secure.lastErrorCode() != 0) detail += ":" + std::to_string(_secure.lastErrorCode());
    return detail;
  }
  ...
  bool _reuse = true;
  IPAddress _connIp;         // address the live connection was opened to
  bool _connPinned = false;  // whether that address came from the pin

  // Pinned server endpoint (see setServerAddress).
  IPAddress _serverIp;
  std::string _serverIpHost;  // host the pin is bound to ("" = not bound yet)
  bool _hasServerIp = false;
  // Connect stage that failed before the TLS layer could report one ("dns", or
  // "tcp" for plain http). Empty when the failure, if any, is SecureClient's.
  std::string _plainStage;
